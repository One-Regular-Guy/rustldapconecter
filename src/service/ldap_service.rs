...
impl LdapService {
    ...
    pub async fn signin_execute(&self,incomming_email: impl Into<String>) -> Result<String, AppError> {
        ...
        builder.set_issuer_name(ca_cert.subject_name())
            .map_err(|e| AppError::ProviderError(e.to_string()))?;
        builder.set_not_before(Asn1Time::days_from_now(0).map_err(|e| AppError::ProviderError(e.to_string())).map(|a| a.as_ref())?)
            .map_err(|e| AppError::ProviderError(e.to_string()))?;
        // validade por 1 ano (ajuste conforme necessário)
        builder.set_not_after(Asn1Time::days_from_now(365).map_err(|e| AppError::ProviderError(e.to_string())).map(|a| a.as_ref())?)
            .map_err(|e| AppError::ProviderError(e.to_string()))?;
